    ...
    m_useIdealRrc (false)
{
  ...
  // TODO Add the possibility to set the type of the antennas
...
MmWaveHelper::GetTypeId (void)
{
  static TypeId
    ...
    .AddAttribute ("Beamformer",
                   ...
                   StringValue ("ns3::MmWaveMMSESpectrumBeamforming"),
...
//      Ptr<MmWaveFFTCodebookBeamforming> bfModule = CreateObjectWithAttributes<MmWaveFFTCodebookBeamforming> ("MobilityModel", PointerValue (mm), "AntennaArray", PointerValue (antenna));
      ...
      //          CreateObjectWithAttributes<MmWaveMMSEBeamforming> ("MobilityModel", PointerValue (mm), "AntennaArray", PointerValue (antenna));
...
MmWaveHelper::InstallSingleEnbDevice (Ptr<Node> n)
{
  ...
  //TODO verify a second time that this merges Tommaso's work with multilayer
  for (std::map<uint8_t,Ptr<MmWaveComponentCarrierEnb> >::iterator it = ccMap.begin (); it != ccMap.end (); ++it)
    {
      ...
      uint8_t numEnbLayers = it->second->GetConfigurationParameters ()->GetNumEnbLayers();
      NS_LOG_INFO ("No. of gNB layers = " << numEnbLayers);
      ...
      for (uint8_t ind=0; ind < numEnbLayers; ind++)
        {
          ...
          //NS_LOG_UNCOND ("DL spectrumphy size: " << dlPhyList.size());
        }
      ...
      bfModule->SetPropagationLossModel( m_pathlossModel.at (it->first)->GetObject<PropagationLossModel> () );
      bfModule->SetSpectrumPropagationLossModel( m_channel.at (it->first)->GetSpectrumPropagationLossModel ());//used to compute BF vectors in some cases
      bfModule->SetConfigurationParameters( it->second->GetConfigurationParameters () );//used to compute BF vectors in some cases
      //dlPhy->SetBeamformingModel (bfModule);
      for (itSpec = dlPhyList.begin (); itSpec != dlPhyList.end (); itSpec++)
        {
  ...
  for (std::map<uint8_t,Ptr<MmWaveComponentCarrierEnb> >::iterator it = ccMap.begin (); it != ccMap.end (); ++it)
    {
      ...
      for (uint8_t layerInd=0; layerInd < numEnbLayers; layerInd++)
        {
          //std::vector<Ptr<MmWaveSpectrumPhy> > tempUl = ccPhy->GetUlSpectrumPhyList ();
          //std::vector<Ptr<MmWaveSpectrumPhy> > tempDl = ccPhy->GetDlSpectrumPhyList ();
          (ccPhy->GetUlSpectrumPhyList ()).at(layerInd)->SetDevice(device);
          (ccPhy->GetDlSpectrumPhyList ()).at(layerInd)->SetDevice(device);
  ...
  for (std::map<uint8_t,Ptr<MmWaveComponentCarrierEnb> >::iterator it = ccMap.begin (); it != ccMap.end (); ++it)
    {
      ...
      for (uint8_t layerInd = 0; layerInd < numEnbLayers; layerInd++)
        {
          m_channel.at (it->first)->AddRx (it->second->GetPhy ()->GetDlSpectrumPhyList ().at(layerInd)); //TODO check if Dl and Ul are the same
        }
    }
...
{
  NS_LOG_FUNCTION_NOARGS ();
  Config::Connect ("/NodeList/*/DeviceList/*/ComponentCarrierMap/*/MmWaveEnbPhy/DlSpectrumPhyList/*/RxPacketTraceEnb",
                   MakeBoundCallback (&MmWavePhyRxTrace::RxPacketTraceEnbCallback, m_phyStats));
  //TODO: in principle we have removed all instances where the eNB phy uses the variable DlSpectrumPhy, using DlSpectrumPhyList/0 instead.
  // The following line is commented-out but left in the code for future reference in case we need to revise this change
