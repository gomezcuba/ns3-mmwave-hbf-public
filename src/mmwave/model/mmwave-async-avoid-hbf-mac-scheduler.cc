...
MmWaveInterAvoidHbfMacScheduler::DoSchedTriggerReq (const struct MmWaveMacSchedSapProvider::SchedTriggerReqParameters& params)
{
  ...
  std::map <uint16_t, struct UeSchedInfo>::iterator itUeInfo, itUeInfo2;
  ...
  if (ueInfo.size () > 0)
    {
	  ...
	  for (itUeInfo = ueInfo.begin (); itUeInfo != ueInfo.end (); itUeInfo++)
	  {
		    if (ueToLayerMapDl.find(itUeInfo->first) != ueToLayerMapDl.end())
		    {
		        int layer = ueToLayerMapDl[itUeInfo->first]; // Obtener el valor del layer del mapa
		        itUeInfo->second.m_dlHbfLayer = layer; // Asignar el valor del layer al usuario del iterador
		    }
	  }
	  //return;		// PARA PROBAS
