								    ...
								    uint8_t rxLayerInd) const
{
  ...
  (*rxPsd) *= (*bfGainPsd);
  NS_LOG_LOGIC("BF Gain TxId " << a->GetObject<Node>()->GetId () << " RxId " << b->GetObject<Node>()->GetId () << " TxBeam " << AntennaArrayBasicModel::GetBeamId(txW) << " RxBeam " << AntennaArrayBasicModel::GetBeamId(rxW) << " g= " << Sum(*bfGainPsd) / bfGainPsd->GetSpectrumModel()->GetNumBands() );
  return rxPsd;
}
